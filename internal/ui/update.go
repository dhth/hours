	...
	case tea.KeyMsg:
		...
		case "left", "h":
			if !m.busy {
				...
				cmds = append(cmds, getRecordsData(m.typ, m.db, m.style, m.period, newStart, newEnd, m.activeFilter, numDays, m.plain))
				m.busy = true
			}
		case "right", "l":
			if !m.busy {
		...
		case "ctrl+t":
			if !m.busy {
				...
				cmds = append(cmds, getRecordsData(m.typ, m.db, m.style, m.period, start, end, m.activeFilter, numDays, m.plain))
				m.busy = true
			}
		}
